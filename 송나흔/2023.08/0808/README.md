...
- n = 1 → 4 / n = 2 → 8 / n = 3 → 12 이니, 가장 아랫부분 정사각형 개수가 n이면 실선으로 이루어진 도형의 둘레의 길이는 4n인 것을 알 수 있음.
- 입력이 10^9까지 주어질 수 있으니 input() 대신 sys.stdin.readline()을 이용하여 입력받음
- 짱 쉬움!
...
- x와 y좌표들을 각각 리스트에 넣고, x와 y 각각에서 max값과 min값 차의 절댓값을 곱하면 끝
- 짱 쉬움~~ 