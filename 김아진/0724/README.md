...
# 풀이 접근
1. 임의의 정답(123~987)을 생성
2. 각 케이스에서 임의의 정답의 strike, ball 구하기
3. strike와 ball이 입력으로 받은 s,b와 비교
    - 같다면 진짜 정답
    - 다르다면 오답
> (수정) 임의의 정답인 num_list의 범위를 점점 줄여가며 최종 정답 도출

# 학습 내용
1. in, not in: 포함 연산자
2. slice: 객체(리스트, 튜플, 문자열)의 범위를 정해 선택 해 새로운 객체 생성
3. set: 중복x, 순서x
4-1. all: 모든 요소가 참이면 참 반환
4-2. any: 요소 중 하나라도 참이면 참 반환