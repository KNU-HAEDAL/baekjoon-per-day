...
1. 주어진 학점과 점수를 리스트로 선언한다.
2. 총 학점을 score_total로 선언하고, 0으로 초기화 한다.
3. 문제에서 주어진 '전공평점은 전공과목별 (학점 × 과목평점)의 합을 학점의 총합으로 나눈 값이다.' 라는 조건을 total로 선언하고, 0으로 초기화 한다.
4. 문제에서 총 20줄이 입력으로 주어진다 했으므로, for문을 20번 반복한다.
5. 입력예시에서 (과목명 점수 학점) 으로 되어있으므로, 각각의 변수를 subject, s, g로 받기위해 띄어쓰기 단위로 입력을 받는다.
6. 'P/F 과목의 경우 등급이 P또는 F로 표시되는데, 등급이 P인 과목은 계산에서 제외해야 한다.' 라는 조건을 만족하기 위해, 등급이 P가 아닐때, if문이 돌아갈 수 있도록 한다.
7. 출력 예시에 맞게 소수점 6자리만 출력하기 위해, 계산 결과를 round(결과, 6)으로 6자리를 줄력한다.
